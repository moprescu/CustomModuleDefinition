...
# showcase mode.
showcaseCodeTabs <- function(rFiles) {
  #rFiles <- list.files(pattern = "testScript.[rR]$")
          ...
          div(class=paste("tab-pane",
              ...
              pre(class="shiny-code",
                  ...
                  HTML(format(tags$code(
                    class="language-r",
                    paste(readLines(file.path(getwd(), rFile)),  collapse="\n")
                  ), indent = FALSE))))
        }))))
}
